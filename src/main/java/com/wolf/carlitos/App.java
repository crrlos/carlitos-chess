    ...
    private static void IniciarFen(String fila, int i){
        ...
        int j = 0;
                ...
                for(char c : fila.toCharArray()){
                    if(c == 'k') Juego.tablero[i][j] = new Rey(false);
                    if(c == 'q') Juego.tablero[i][j] = new Dama(false);
                    if(c == 'r') Juego.tablero[i][j] = new Torre(false);
                    if(c == 'b') Juego.tablero[i][j] = new Alfil(false);
                    if(c == 'n') Juego.tablero[i][j] = new Caballo(false);
                    if(c == 'p') Juego.tablero[i][j] = new Peon(false);
                    
                    if(c == 'K') Juego.tablero[i][j] = new Rey(true);
                    if(c == 'Q') Juego.tablero[i][j] = new Dama(true);
                    if(c == 'R') Juego.tablero[i][j] = new Torre(true);
                    if(c == 'B') Juego.tablero[i][j] = new Alfil(true);
                    if(c == 'N') Juego.tablero[i][j] = new Caballo(true);
                    if(c == 'P') Juego.tablero[i][j] = new Peon(true);
                    
                    if(Character.isDigit(c)) j += Integer.parseInt(String.valueOf(c)); else j++;
                }
